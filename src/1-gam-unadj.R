...
d <- read.csv(paste0(dropboxDir,"Data/Cleaned/Audrie/bangladesh-dm-ee-ee-growth-stool-urine-lab-covariates-anthro.csv"))
...
#H1: EED biomarkers will be negatively associated with concurrent 
#LAZ, WAZ, WLZ, and head-circumference for age Z-score at 3 months, 14 months and 28 months.

#t1
Xvars <- c("ln_aat1", "ln_mpo1", "ln_neo1", "ln_L_conc_t1", "ln_M_conc_t1")            
Yvars <- c("laz_t1", "waz_t1", "whz_t1" ,"hcz_t1") 
...
for(i in Xvars){
  for(j in Yvars){
    ...
    res <- data.frame(X=i, Y=j, N=res_unadj$n, 
                      fit=I(list(res_unadj$fit)), dat=I(list(res_unadj$dat)))
    H1_models <- bind_rows(H1_models, res)
  }
}

#t2
Xvars <- c("ln_aat2", "ln_mpo2", "ln_neo2", "ln_L_conc_t2", "ln_M_conc_t2")            
...
#t3
Xvars <- c("ln_aat3", "ln_mpo3", "ln_neo3", "ln_L_conc_t3", "ln_M_conc_t3")            
...
for(i in 1:nrow(H1_models)){
  ...
  preds <- predict_gam_diff(fit=H1_models$fit[i][[1]], d=H1_models$dat[i][[1]], 
                            H1_models$N[i], quantile_diff=c(0.25,0.75), 
                            Xvar=res$X, Yvar=res$Y)
...
for(i in 1:nrow(H2_models)){
  ...
  preds <- predict_gam_diff(fit=H2_models$fit[i][[1]], d=H2_models$dat[i][[1]], 
                            H2_models$N[i], quantile_diff=c(0.25,0.75), 
                            Xvar=res$X, Yvar=res$Y)
...
for(i in 1:nrow(H3_models)){
  ...
  preds <- predict_gam_diff(fit=H3_models$fit[i][[1]], d=H3_models$dat[i][[1]], 
                            H3_models$N[i], quantile_diff=c(0.25,0.75), 
                            Xvar=res$X, Yvar=res$Y)
...
Xvars <- c("ln_aat1", "ln_mpo1", "ln_neo1", "ln_L_conc_t1", "ln_M_conc_t1")           
Yvars <- c(names(select(d, grep("velocity", names(d), value = T)))) 
...
for(i in 1:nrow(H4_models)){
  ...
  preds <- predict_gam_diff(fit=H4_models$fit[i][[1]], d=H4_models$dat[i][[1]], 
                            H4_models$N[i], quantile_diff=c(0.25,0.75), 
                            Xvar=res$X, Yvar=res$Y)
...
Xvars <- c("ln_aat2", "ln_mpo2", "ln_neo2", "ln_L_conc_t2", "ln_M_conc_t2")           
Yvars <- c(names(select(d, grep("velocity_t2_t3", names(d), value = T)))) 
...
for(i in 1:nrow(H5_models)){
  ...
  preds <- predict_gam_diff(fit=H5_models$fit[i][[1]], d=H5_models$dat[i][[1]], 
                            H5_models$N[i], quantile_diff=c(0.25,0.75), 
                            Xvar=res$X, Yvar=res$Y)
