...
bf_full <- bf_full %>% rename("bf_t1"="c605.x", "bf_t2"="c605.y", "bf_t3"="c605") %>%
  mutate(bf_t1 = factor(bf_t1, labels = c("Breastfeeding", "Weaned")), 
         bf_t2 = factor(bf_t2, labels = c("Breastfeeding", "Weaned")),
         bf_t3 = factor(bf_t3, labels = c("Breastfeeding", "Weaned"))) 
...
add_bf <- function(x, y, W){
  if(grepl("t1", x)|grepl("t1",y)) {
    W <- c(W, "bf_t1")
  }
  if(grepl("t2",x)|grepl("t2",y)) {
    W <- c(W, "bf_t2")
  }
  if(grepl("t3",x)|grepl("t3",y)){
    W <- c(W, "bf_t3")
  }
  W
}
...
saveRDS(total, here("Bangladesh/results/adjusted/bf/all_results_sens_bf.RDS"))
saveRDS(H1_adj_res_BH, here("Bangladesh/results//adjusted/bf/H1_adj_res.RDS"))
saveRDS(H2_adj_res_BH, here("Bangladesh/results/adjusted/bf/H2_adj_res.RDS"))
saveRDS(H3_adj_res_BH, here("Bangladesh/results/adjusted/bf/H3_adj_res.RDS"))
saveRDS(H4_adj_res_BH, here("Bangladesh/results/adjusted/bf/H4_adj_res.RDS"))
saveRDS(H5_adj_res_BH, here("Bangladesh/results/adjusted/bf/H5_adj_res.RDS"))
