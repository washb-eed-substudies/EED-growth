        ...
        for (l in 1:nrow(filtered_adj)) {
          ...
          if(l == 1){
            tbl <- rbind(tbl, list(s_name, e_name, out_var[j], filtered_adj$N[l], level,
                                   v, round(filtered_adj$Pval[l], 2), round(filtered_adj$BH.Pval[l], 2),"",""))
            ...
            tbl <- rbind(tbl, list(" ", " ", " ", " ", level, 
                                   v, round(filtered_adj$Pval[l], 2), round(filtered_adj$BH.Pval[l], 2), 
                                   round(has_int_pval$int.Pval[1], 2), round(has_int_pval$BH.int.Pval[1], 2)))
          }
        }
      }
...
hr_tbl_flex <- function(name, expo_var, out_var, exposure, outcome, results, results_adj, adj_only=F, exp_col_size = 1, out_col_size = 1){
  ...
  if(adj_only){
    ...
    flextbl <- fontsize(flextbl, part = "all", size = 7)
    flextbl <- width(flextbl, 1:7, width=c(exp_col_size, out_col_size, .3, .55, .55, 1, .5))
